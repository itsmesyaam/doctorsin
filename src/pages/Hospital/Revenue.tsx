...
export const Revenue: React.FC = () => {
  ...
  return (
    <div className="space-y-8 text-left">
      {/* Title */}
      <div>
        <h2 className="text-2xl font-black text-slate-800 tracking-tight">Revenue & Payout Roster</h2>
        <p className="text-xs text-slate-500 mt-1">Review gross payouts, transaction logs, commission splits, and bank deposits.</p>
      </div>
      ...
      <div className="grid sm:grid-cols-3 gap-6">
        <div className="bg-white border border-slate-200/80 p-5 rounded-2xl">
          ...
          <span className="text-xl font-black text-slate-800 block mt-1">${totalRev.toLocaleString()}</span>
        </div>
        <div className="bg-white border border-slate-200/80 p-5 rounded-2xl">
          <span className="text-[10px] text-slate-400 font-bold block uppercase">Commission Split (15%)</span>
          <span className="text-xl font-black text-slate-800 block mt-1">${(totalRev * 0.15).toLocaleString()}</span>
        </div>
        <div className="bg-white border border-slate-200/80 p-5 rounded-2xl">
          <span className="text-[10px] text-slate-400 font-bold block uppercase">Net Bank Deposits</span>
          <span className="text-xl font-black text-emerald-600 block mt-1">${(totalRev * 0.85).toLocaleString()}</span>
        </div>
      </div>
      ...
      <div className="bg-white border border-slate-200/80 rounded-3xl overflow-hidden shadow-sm">
        <div className="p-6 border-b border-slate-105">
          <h3 className="font-bold text-slate-850 text-sm">Recent Consultation Bills</h3>
        </div>
            ...
            <tbody className="divide-y divide-slate-100 font-medium">
              {transactions.map((t) => (
                <tr key={t.id} className="hover:bg-slate-50/50">
                  ...
                  <td className="p-4 font-bold text-slate-800">{t.patient}</td>
                  <td className="p-4 text-slate-655 font-semibold">{t.dept}</td>
                  <td className="p-4 text-slate-500">{t.date}</td>
                  <td className="p-4 font-black text-slate-800">${t.amount}</td>
                  <td className="p-4">
                    <span className="bg-emerald-50 text-emerald-600 px-2.5 py-0.5 rounded-full text-[10px] font-bold border border-emerald-150">
                      {t.status}
                    </span>
                  </td>
                </tr>
